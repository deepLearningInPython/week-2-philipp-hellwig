...
def dR(beta, x, y):
    N = len(x)
    dbeta_0 = (2/ N) * np.sum(beta[0] + beta[1] * x - y) # implement the above formula for dR/dβ₀
    dbeta_1 = (2/ N) * np.sum((beta[0] + beta[1] * x - y) * x) # implement the above formula for dR/dβ₁
